...
public class TransferCodeBlockGUIScreen extends AbstractContainerScreen<TransferCodeBlockGUIMenu> {
	...
	protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
		guiGraphics.drawString(this.font, Component.translatable("gui.puzzle_code.transfer_code_block_gui.label_from_nbt_bnbttextgetnbt"), 5, 42, -12829636, false);
		guiGraphics.drawString(this.font, Component.translatable("gui.puzzle_code.transfer_code_block_gui.label_to_nbt_bnbttextsetnbt"), 5, 92, -12829636, false);
		guiGraphics.drawString(this.font,

				ReturnGetNBTProcedure.execute(world, x, y, z), 59, 42, -12829636, false);
		guiGraphics.drawString(this.font,

				ReturnSetNBTProcedure.execute(world, x, y, z), 46, 92, -12829636, false);
		guiGraphics.drawString(this.font, Component.translatable("gui.puzzle_code.transfer_code_block_gui.label_nbt_getnbt"), 5, 29, -12829636, false);
		guiGraphics.drawString(this.font, Component.translatable("gui.puzzle_code.transfer_code_block_gui.label_nbt_setnbt"), 5, 78, -12829636, false);
	}
	...
	public void init() {
		super.init();
		getNBTField = new EditBox(this.font, this.leftPos + 6, this.topPos + 7, 111, 18, Component.translatable("gui.puzzle_code.transfer_code_block_gui.getNBTField")) {
			@Override
			public void insertText(String text) {
				super.insertText(text);
				if (getValue().isEmpty())
					setSuggestion(Component.translatable("gui.puzzle_code.transfer_code_block_gui.getNBTField").getString());
				else
					setSuggestion(null);
			}
		...
		getNBTField.setSuggestion(Component.translatable("gui.puzzle_code.transfer_code_block_gui.getNBTField").getString());
		getNBTField.setMaxLength(32767);
		guistate.put("text:getNBTField", getNBTField);
		this.addWidget(this.getNBTField);
		setNBTField = new EditBox(this.font, this.leftPos + 6, this.topPos + 57, 111, 18, Component.translatable("gui.puzzle_code.transfer_code_block_gui.setNBTField")) {
			@Override
			public void insertText(String text) {
				super.insertText(text);
				if (getValue().isEmpty())
					setSuggestion(Component.translatable("gui.puzzle_code.transfer_code_block_gui.setNBTField").getString());
				else
					setSuggestion(null);
			}
		...
		setNBTField.setSuggestion(Component.translatable("gui.puzzle_code.transfer_code_block_gui.setNBTField").getString());
