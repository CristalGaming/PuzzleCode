...
public class TeleporterLogicCodeBlockGUI2Screen extends AbstractContainerScreen<TeleporterLogicCodeBlockGUI2Menu> {
	...
	protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
		guiGraphics.drawString(this.font, Component.translatable("gui.puzzle_code.teleporter_logic_code_block_gui_2.label_from_nbt_bnbttextgetnbt"), 6, 42, -12829636, false);
		guiGraphics.drawString(this.font, Component.translatable("gui.puzzle_code.teleporter_logic_code_block_gui_2.label_to_nbt_bnbttextsetnbt"), 6, 91, -12829636, false);
		...
		guiGraphics.drawString(this.font, Component.translatable("gui.puzzle_code.teleporter_logic_code_block_gui_2.label_nbt_setnbt"), 6, 78, -12829636, false);
		guiGraphics.drawString(this.font, Component.translatable("gui.puzzle_code.teleporter_logic_code_block_gui_2.label_nbtgetnbt"), 6, 28, -12829636, false);
	}
	...
	public void init() {
		super.init();
		getNBTField = new EditBox(this.font, this.leftPos + 7, this.topPos + 7, 111, 18, Component.translatable("gui.puzzle_code.teleporter_logic_code_block_gui_2.getNBTField")) {
			@Override
			public void insertText(String text) {
				super.insertText(text);
				if (getValue().isEmpty())
					setSuggestion(Component.translatable("gui.puzzle_code.teleporter_logic_code_block_gui_2.getNBTField").getString());
				else
					setSuggestion(null);
			}
		...
		getNBTField.setSuggestion(Component.translatable("gui.puzzle_code.teleporter_logic_code_block_gui_2.getNBTField").getString());
		getNBTField.setMaxLength(32767);
		guistate.put("text:getNBTField", getNBTField);
		this.addWidget(this.getNBTField);
		setNBTField = new EditBox(this.font, this.leftPos + 7, this.topPos + 56, 111, 18, Component.translatable("gui.puzzle_code.teleporter_logic_code_block_gui_2.setNBTField")) {
			@Override
			public void insertText(String text) {
				super.insertText(text);
				if (getValue().isEmpty())
					setSuggestion(Component.translatable("gui.puzzle_code.teleporter_logic_code_block_gui_2.setNBTField").getString());
				else
					setSuggestion(null);
			}
		...
		setNBTField.setSuggestion(Component.translatable("gui.puzzle_code.teleporter_logic_code_block_gui_2.setNBTField").getString());
		setNBTField.setMaxLength(32767);
		guistate.put("text:setNBTField", setNBTField);
		this.addWidget(this.setNBTField);
		button_1 = Button.builder(Component.translatable("gui.puzzle_code.teleporter_logic_code_block_gui_2.button_1"), e -> {
			if (true) {
		...
		}).bounds(this.leftPos + 65, this.topPos + 109, 18, 20).build();
		guistate.put("button:button_1", button_1);
		this.addRenderableWidget(button_1);
		button_2 = Button.builder(Component.translatable("gui.puzzle_code.teleporter_logic_code_block_gui_2.button_2"), e -> {
			if (true) {
		...
		}).bounds(this.leftPos + 92, this.topPos + 109, 18, 20).build();
		...
		imagebutton_enter_button1 = new ImageButton(this.leftPos + 146, this.topPos + 55, 20, 20, 0, 0, 20, new ResourceLocation("puzzle_code:textures/screens/atlas/imagebutton_enter_button1.png"), 20, 40, e -> {
			if (true) {
				PuzzleCodeMod.PACKET_HANDLER.sendToServer(new TeleporterLogicCodeBlockGUI2ButtonMessage(5, x, y, z));
				TeleporterLogicCodeBlockGUI2ButtonMessage.handleButtonAction(entity, 5, x, y, z);
			}
		});
