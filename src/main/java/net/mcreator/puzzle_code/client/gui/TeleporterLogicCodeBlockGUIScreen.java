...
public class TeleporterLogicCodeBlockGUIScreen extends AbstractContainerScreen<TeleporterLogicCodeBlockGUIMenu> {
	...
	private static final ResourceLocation texture = new ResourceLocation("puzzle_code:textures/teleporter_logic_code_block_gui.png");
	...
	public void init() {
		...
		this.addRenderableWidget(new Button(this.leftPos + 24, this.topPos + 119, 13, 20, new TextComponent("2"), e -> {
			if (true) {
				PuzzleCodeMod.PACKET_HANDLER.sendToServer(new TeleporterLogicCodeBlockGUIButtonMessage(3, x, y, z));
				TeleporterLogicCodeBlockGUIButtonMessage.handleButtonAction(entity, 3, x, y, z);
			}
		}));
		this.addRenderableWidget(new Button(this.leftPos + -42, this.topPos + 6, 45, 20, new TextComponent("Edit"), e -> {
			if (true) {
				PuzzleCodeMod.PACKET_HANDLER.sendToServer(new TeleporterLogicCodeBlockGUIButtonMessage(4, x, y, z));
				TeleporterLogicCodeBlockGUIButtonMessage.handleButtonAction(entity, 4, x, y, z);
			}
		}));
		this.addRenderableWidget(new Button(this.leftPos + -42, this.topPos + 42, 45, 20, new TextComponent("Edit"), e -> {
			if (true) {
				PuzzleCodeMod.PACKET_HANDLER.sendToServer(new TeleporterLogicCodeBlockGUIButtonMessage(5, x, y, z));
				TeleporterLogicCodeBlockGUIButtonMessage.handleButtonAction(entity, 5, x, y, z);
			}
		}));
		this.addRenderableWidget(new Button(this.leftPos + -42, this.topPos + 78, 45, 20, new TextComponent("Edit"), e -> {
			if (true) {
				PuzzleCodeMod.PACKET_HANDLER.sendToServer(new TeleporterLogicCodeBlockGUIButtonMessage(6, x, y, z));
				TeleporterLogicCodeBlockGUIButtonMessage.handleButtonAction(entity, 6, x, y, z);
			}
		}));
	}
}
