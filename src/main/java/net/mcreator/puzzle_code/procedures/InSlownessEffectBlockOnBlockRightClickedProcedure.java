...
public class InSlownessEffectBlockOnBlockRightClickedProcedure {
	public static InteractionResult execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return InteractionResult.PASS;
		if (PuzzleCodeModItems.GUI_SET.get() == (entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem()) {
			{
				if (entity instanceof ServerPlayer _ent) {
					...
					NetworkHooks.openGui((ServerPlayer) _ent, new MenuProvider() {
						...
						public Component getDisplayName() {
							return new TextComponent("InEffectBlockGUI1");
						}

						@Override
						public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
							return new InEffectBlockGUI1Menu(id, inventory, new FriendlyByteBuf(Unpooled.buffer()).writeBlockPos(_bpos));
						}
					}, _bpos);
				}
			}
		...
		}.getValue(world, new BlockPos(x, y, z), "idDisabled"))) {
			if (entity instanceof LivingEntity _entity)
				_entity.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, (int) (new Object() {
