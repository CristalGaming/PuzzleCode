...
import net.mcreator.puzzle_code.network.MessagerBlockGUIButtonMessage;
...
public class MessagerBlockGUIScreen extends AbstractContainerScreen<MessagerBlockGUIMenu> {
	private final static HashMap<String, Object> guistate = MessagerBlockGUIMenu.guistate;
	private final Level world;
	private final int x, y, z;
	private final Player entity;
	EditBox textCodeBlockField;

	public MessagerBlockGUIScreen(MessagerBlockGUIMenu container, Inventory inventory, Component text) {
	...
	private static final ResourceLocation texture = new ResourceLocation("puzzle_code:textures/screens/messager_block_gui.png");
	...
	protected void renderBg(PoseStack ms, float partialTicks, int gx, int gy) {
		...
		this.blit(ms, this.leftPos + 150, this.topPos + 4, 0, 0, -1, -1, -1, -1);
	...
	public void init() {
		...
		this.addRenderableWidget(new Button(this.leftPos + 6, this.topPos + 31, 77, 20, new TextComponent("Apply"), e -> {
			if (true) {
				PuzzleCodeMod.PACKET_HANDLER.sendToServer(new MessagerBlockGUIButtonMessage(0, x, y, z));
				MessagerBlockGUIButtonMessage.handleButtonAction(entity, 0, x, y, z);
			}
		}));
		this.addRenderableWidget(new Button(this.leftPos + 92, this.topPos + 31, 76, 20, new TextComponent("Edit"), e -> {
			if (true) {
				PuzzleCodeMod.PACKET_HANDLER.sendToServer(new MessagerBlockGUIButtonMessage(1, x, y, z));
				MessagerBlockGUIButtonMessage.handleButtonAction(entity, 1, x, y, z);
			}
		}));
	}
}
