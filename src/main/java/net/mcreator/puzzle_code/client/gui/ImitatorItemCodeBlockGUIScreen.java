...
public class ImitatorItemCodeBlockGUIScreen extends AbstractContainerScreen<ImitatorItemCodeBlockGUIMenu> {
	...
	private static final ResourceLocation texture = new ResourceLocation("puzzle_code:textures/imitator_item_code_block_gui.png");
	...
	public void init() {
		...
		this.addRenderableWidget(new Button(this.leftPos + 24, this.topPos + 119, 14, 20, new TextComponent("2"), e -> {
			if (true) {
				PuzzleCodeMod.PACKET_HANDLER.sendToServer(new ImitatorItemCodeBlockGUIButtonMessage(3, x, y, z));
				ImitatorItemCodeBlockGUIButtonMessage.handleButtonAction(entity, 3, x, y, z);
			}
		}));
		this.addRenderableWidget(new Button(this.leftPos + -42, this.topPos + 6, 45, 20, new TextComponent("Edit"), e -> {
			if (true) {
				PuzzleCodeMod.PACKET_HANDLER.sendToServer(new ImitatorItemCodeBlockGUIButtonMessage(4, x, y, z));
				ImitatorItemCodeBlockGUIButtonMessage.handleButtonAction(entity, 4, x, y, z);
			}
		}));
		this.addRenderableWidget(new Button(this.leftPos + -42, this.topPos + 42, 45, 20, new TextComponent("Edit"), e -> {
			if (true) {
				PuzzleCodeMod.PACKET_HANDLER.sendToServer(new ImitatorItemCodeBlockGUIButtonMessage(5, x, y, z));
				ImitatorItemCodeBlockGUIButtonMessage.handleButtonAction(entity, 5, x, y, z);
			}
		}));
		this.addRenderableWidget(new Button(this.leftPos + -42, this.topPos + 78, 45, 20, new TextComponent("Edit"), e -> {
			if (true) {
				PuzzleCodeMod.PACKET_HANDLER.sendToServer(new ImitatorItemCodeBlockGUIButtonMessage(6, x, y, z));
				ImitatorItemCodeBlockGUIButtonMessage.handleButtonAction(entity, 6, x, y, z);
			}
		}));
	}
}
